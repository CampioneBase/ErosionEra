...
public class ErosionCommand {
    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher){
        ...
        dispatcher.register(literal(ENTER_TEXT).requires(source -> source.hasPermissionLevel(DATA_COMMAND_LEVEL)).then(literal(KEY_COMMAND)
                ...
                .then(literal("clear")
                        .then(CommandManager.argument("player", EntityArgumentType.player())
                                .executes(source -> {
                                    getPlayerData(EntityArgumentType.getPlayer(source, "player")).clear();
                                    return Command.SINGLE_SUCCESS;
                                })
                        )
                        .executes(source -> {
                            getPlayerData(source.getSource().getPlayer()).clear();
                            return Command.SINGLE_SUCCESS;
                        })
                )
                ...
                .then(literal("clean")
                        .then(CommandManager.argument("player", EntityArgumentType.player())
                                .executes(source -> {
                                    getPlayerData(EntityArgumentType.getPlayer(source, "player")).clean();
                                    return Command.SINGLE_SUCCESS;
                                })
                        )
                        .executes(source -> {
                            getPlayerData(source.getSource().getPlayer()).clean();
                            return Command.SINGLE_SUCCESS;
                        })
                )
                        ...
                        .then(CommandManager.argument("value", IntegerArgumentType.integer())
                                .then(CommandManager.argument("player", EntityArgumentType.player())
                                        .executes(source -> {
                                            ...
                                            getPlayerData(player).add(value);
                                ...
                                .then(CommandManager.argument("type", ErosionChangeArgumentType.create())
                                        .then(CommandManager.argument("player", EntityArgumentType.player())
                                                .executes(source -> {
                                                    ...
                                                    getPlayerData(player).levelUp(type);
                                ...
                                .then(CommandManager.argument("type", ErosionChangeArgumentType.create())
                                        .then(CommandManager.argument("player", EntityArgumentType.player())
                                                .executes(source -> {
                                                    PlayerEntity player = EntityArgumentType.getPlayer(source, "player");
                                                    ErosionChangeType type = ErosionChangeArgumentType.get(source, "type");
                                                    getPlayerData(player).levelDown(type);
    ...
    private static int showData(CommandContext<ServerCommandSource> source, PlayerEntity player) throws CommandSyntaxException {
        ErosionData data = getPlayerData(player);
        source.getSource().sendMessage(Text.of("玩家 " +
                player.getName().getString() +
                " 侵蚀度为 " +
                data.getErosion() +
                " 侵蚀等级为 " +
                data.getErosionLevel()
        ));
        return Command.SINGLE_SUCCESS;
    }

    private static ErosionData getPlayerData(PlayerEntity player) throws CommandSyntaxException {
        if (player == null) throw new SimpleCommandExceptionType(Text.of("不存在玩家实体")).create();
        return ErosionData.get(player);
    }
}
