...
import lunastic.erosion_era.init.StatusEffects;
...
import static lunastic.erosion_era.util.data.PlayerExtraData.LOGGER;
...
public final class ErosionData implements DataCompound {
    ...
    public void levelUp(){
        this.levelUp(ErosionChangeType.PROGRESS);
    }
    ...
    public void levelDown(){
        this.levelDown(ErosionChangeType.PROGRESS);
    }
    ...
    public void tick(PlayerEntity player) {
        if(player.hasStatusEffect(StatusEffects.EROSION_EFFECT)){
            StatusEffectInstance sei = player.getStatusEffect(StatusEffects.EROSION_EFFECT);
            if(sei == null) return;
            // todo 侵蚀状态与侵蚀数据影响
            // 目前还不清楚为啥这个BUFF持续时间结束后不能自动消除
            // 在此处手动添加消除
            if(sei.getDuration() <= 0) player.removeStatusEffect(StatusEffects.EROSION_EFFECT);
        }
    }
